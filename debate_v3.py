...
async def run_debate_logic(question: str, top_k: int, max_rounds: int, output: str, verbose: bool):
    ...
    if final_summary != "Skipped - no merged factors." and final_summary != "Summary could not be generated.":
        with Progress(SpinnerColumn(), TextColumn("[progress.description]{task.description}"), console=console, transient=True) as progress:
            refine_task = progress.add_task("[yellow]Integrating debate insights into baseline...", total=None)
        ...
        console.print("\n[bold blue][Refined Answer (Integrated)][/bold blue]")
        console.print(refined_answer)
    else:
        # If summary failed or was skipped, refinement doesn't make sense.
        # Use baseline as the candidate for the judge instead.
        console.print("\n[yellow]Skipping refinement step as debate summary was not generated.[/yellow]")
    ...
    if judge_decision == "Accept Merged": # Interpretation: Judge accepts the refined answer over baseline
        console.print("[green]Judge approved refined answer.[/green]")
        final_output = refined_answer # <-- Use refined_answer
        transcript_data["final_decision"] = "Accepted Refined"
        ...
        console.print(f"[bold red]Judge resulted in error:[/bold red] {judge_decision}. Defaulting to refined answer.")
        final_output = refined_answer # <-- Default to refined_answer on judge error
        transcript_data["final_decision"] = f"Error ({judge_decision}) - Used Refined"
